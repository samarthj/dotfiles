...
lsp_config.tsserver.setup {
    ...
    root_dir = root_pattern('package.json', 'tsconfig.json', 'jsconfig.json', '.git', vim.fn.getcwd())
}
...
lsp_config.sumneko_lua.setup {
    cmd = {'lua-language-server', '-E', '/usr/share/lua-language-server/main.lua', '--logpath',
...
lsp_config.hls.setup {
    ...
    root_dir = root_pattern('*.cabal', 'stack.yaml', 'cabal.project', 'package.yaml', 'hie.yaml', vim.fn.getcwd())
}
...
lsp_config.clangd.setup {
    ...
    root_dir = root_pattern('CMakeLists.txt', 'compile_flags.txt', '.git', vim.fn.getcwd())
}
...
if vim.fn.has("win64") then
    local jdt_base = vim.fn.expand("~/Projects/eclipse.jdt.ls/org.eclipse.jdt.ls.product/target/repository")
    ...
    lsp_config.jdtls.setup {
        cmd = {vim.fn.expand(java_home .. "/bin/java.exe"), "-Declipse.application=org.eclipse.jdt.ls.core.id1",
               "-Dosgi.bundles.defaultStartLevel=4", "-Declipse.product=org.eclipse.jdt.ls.core.product",
               "-Dlog.protocol=true", "-Dlog.level=ALL", "-Xmx1G", "-jar",
               vim.fn.expand(jdt_base .. "/plugins/org.eclipse.equinox.launcher_1.6.200.v20210416-2027.jar"),
               "-configuration", vim.fn.expand(jdt_base .. "/config_win"), "-data",
               vim.fn.expand("~/Projects/jdt-ls-data"), "--add-modules=ALL-SYSTEM", "--add-opens",
    ...
    lsp_config.kotlin_language_server.setup {
        cmd = {vim.fn.expand("~/Projects/kotlin-language-server/server/build/install" ..
...
local function setup_hover()
    ...
    lsp.handlers[method] = lsp.with(lsp.handlers[method], {
        border = win_border
    })
end
...
local function setup_diagnostics()
    ...
    local diagnostics_handler = lsp.with(lsp.diagnostic.on_publish_diagnostics, on_publish_cfg)
